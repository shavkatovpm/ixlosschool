...
## Admin boshqaradigan kontent va blog (2026-09-24)

- Sahifalar **har so'rovda render qilinadi** (`[locale]/layout.tsx`da `force-dynamic`), chunki aloqa ma'lumoti, FAQ, ustozlar, video fikrlar va maqolalar admin paneldan o'zgaradi (`docs/admin.md`). Panel bo'lmagan joyda (Vercel) kod ichidagi standart kontent ko'rsatiladi. Yangi kontent turi qo'shilsa: `src/lib/content/` orqali o'qing (xatoda standartga qayting), sahifani statik qilmang.
- Aloqa/yuridik ma'lumot: `src/lib/center.ts` (`getContact()`, `getLegal()`); komponentlarga to'g'ridan-to'g'ri `CONTACT`/`LEGAL` import qilmang (ular faqat standart qiymat). Klient komponentlar telefonni `useContact()` (ContactProvider) orqali oladi.
- **Blog:** `/[locale]/blog` va `/[locale]/blog/[slug]`. Maqola faqat uch tilda ham to'liq bo'lsa chop etiladi; hreflang, canonical, BlogPosting + BreadcrumbList JSON-LD, sitemap va `llms.txt`ga avtomatik qo'shiladi. Maqola yo'q bo'lsa `/blog` 404 (yupqa sahifa yaratmaydi) va footerda havola chiqmaydi. Mavzular: `docs/blog-reja.md`.
- Tekshiruv ro'yxati (chop etishdan oldin): sarlavha ≤ 60 va asosiy so'z boshida, tavsif ≤ 160, faktlar faqat maktab tasdiqlagan (narx, natija, va'da to'qilmaydi), ichki havolalar (Qabul, Ustozlar), uch tilda ham tabiiy matn (mashina tarjimasi emas).

## Tillar va Google, hujjatlar (2026-09-25)

- **Qaysi til ko'rinadi:** Google qidiruvda foydalanuvchining tili/mintaqasiga mos versiyani ko'rsatadi (inglizcha so'rov yoki inglizcha brauzer = `/en`). Bu odatiy. `/uz` ni ustun qilish uchun signallar: hreflang `x-default` → `/uz` (HTML'da), `School`/`WebSite` JSON-LD `url` = `/uz`, `/` → `/uz` **308** (UTM saqlanadi), sitemap'da hreflang. **Hreflang faqat HTML'da:** `routing.ts`da `alternateLinks: false` (next-intl'ning HTTP `Link` sarlavhasi `x-default`ni `/` ga qo'yib HTML bilan zid kelgan edi). Yangi sahifa qo'shsangiz, `buildMetadata()` orqali qo'shing (hreflang o'zi chiqadi).
- Sitemap'da fixed sahifalar uchun `lastmod` yo'q (haqiqiy o'zgarish sanasi kuzatilmaydi; har so'rovda o'zgaradigan sana Google uchun zararli). Maqolalar haqiqiy `updated_at` bilan chiqadi.
- **Footerdagi hujjatlar:** `public/legal/` (guvohnoma va litsenziya: PDF + WebP ko'rinish + kichik rasm). Manba PDF'lar git'ga tushmaydigan `/tasdiqnoma/` papkasida. Hujjat yangilansa: `pdftoppm -r 150 -png` bilan 1-betni rasmga aylantirib, `sharp` bilan WebP (1100 px va 360 px) qiling, fayl nomlari saqlansin (`legal-documents.tsx`, `footer.tsx`, `llms.ts` shularga havola qiladi). Diqqat: litsenziya PDF'ida faoliyat manzili 221-uy, saytda 219-uy (egasi qarori bilan shunday qoldirilgan).
- Ariza formasidagi sinf tanlash: `grade-select.tsx` (popover asosidagi listbox; popover API'siz brauzerlarda oddiy `<select>`).
