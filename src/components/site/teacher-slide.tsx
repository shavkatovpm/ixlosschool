...
export function TeacherSlide({ teacher, hidden = false }: { teacher: Teacher; hidden?: boolean }) {
  ...
  return (
    <Link
      ...
      draggable={false}
      className="group relative block aspect-[4/5] w-[clamp(256px,24vw,332px)] shrink-0 overflow-hidden rounded-[26px] bg-tint-a transition-shadow duration-500 hover:shadow-[0_32px_50px_-28px_rgba(13,39,22,0.75)]"
    >
      <Image
        src={teacher.photo}
      ...
      <div className="absolute inset-x-0 bottom-0 bg-gradient-to-t from-[#0d2716] via-[#0d2716]/85 to-transparent px-5 pb-5 pt-20 text-white">
        ...
        {first ? (
          <p className="mt-3 line-clamp-2 text-[13.5px] leading-[1.45] text-[#dbe6ce]">
            <span className="font-bold text-[#cfdda7]">{first.label}</span> · {first.value}
          </p>
        ) : null}
        ...
        {rest.length > 0 ? (
          <div className="grid grid-rows-[0fr] transition-[grid-template-rows] duration-500 ease-out group-hover:grid-rows-[1fr] group-focus-visible:grid-rows-[1fr]">
            <ul className="min-h-0 overflow-hidden">
              {rest.map((fact) => (
                <li key={`${fact.label}-${fact.value}`} className="pt-2.5 text-[13.5px] leading-[1.45] text-[#dbe6ce]">
                  <span className="font-bold text-[#cfdda7]">{fact.label}</span> · {fact.value}
                </li>
              ))}
            </ul>
          </div>
