...
export default async function LoginPage({ searchParams }: { searchParams: Promise<{ e?: string }> }) {
  ...
  return (
    <main className="flex min-h-screen items-center justify-center px-5 py-12">
      <div className="admin-auth-card w-full max-w-[420px] rounded-[28px] bg-surface p-8 shadow-[0_24px_60px_-28px_rgba(22,46,37,0.35)] sm:p-10">
        ...
        <form action={loginAction} className="mt-6">
          ...
          <button
            type="submit"
            className="admin-primary mt-7 h-12 w-full rounded-[12px] bg-brand text-[15px] font-bold text-white transition-colors hover:bg-brand-soft"
          >
            Kirish
          </button>
        </form>
